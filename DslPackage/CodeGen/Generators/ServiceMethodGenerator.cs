	...
	internal class ServiceMethodGenerator
	{
		...
		internal void GenerateUpdateMethod(EntityModel entity, UpdateMethodModel method, List<string> output, List<string> interfaceOutput)
		{
			...
			foreach (var updProp in updateProps)
			{
				...
				sbSigArgs.Append($", {updProp.PropertyModel.DataType} {updProp.PropertyModel.Name}");
			}
			...
			foreach (var updProp in method.UpdateProperties.Where(p => !p.IsOptional && p.PropertyModel.DataType == "String"))
				output.AddLine(tc + 1, $"ArgumentNullException.ThrowIfNull({updProp.PropertyModel.Name});");
			output.AddLine();
			output.AddLine(tc + 1, "try {");
		...
		{
			if (filterProp.PropertyModel.DataType == "String")
			{
		...
		private void GenerateInternalFilter(FilterPropertyModel filterProp, List<string> output, int tc)
		{
			...
			else
			{
				...
				{
					//output.AddLine(indent, $"dbQuery = dbQuery.Where(x => x.{filterProp.PropertyModel.Name} == {filterProp.PropertyModel.EnumType.Name}.{filterProp.InternalValue});");
		...
		internal void GenerateQueryMethod(EntityModel entity, ReadMethodModel queryMethod, List<string> output, List<string> interfaceOutput)
		{
			...
			foreach (var filterProp in queryMethod.FilterProperties)
			{
				...
				else if (filterProp.PropertyModel.DataType == "Int32" || filterProp.PropertyModel.DataType == "Boolean" || filterProp.PropertyModel.DataType == "Guid")
				{
