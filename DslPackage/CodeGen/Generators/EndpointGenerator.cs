...
namespace Dyvenix.GenIt.DslPackage.CodeGen.Generators
{
	internal class EndpointGenerator
	{
		...
		internal EndpointGenerator(ModelRoot modelRoot, Dictionary<string, ModuleModel> modules)
		{
			// Convenience vars
		...
		private void ResetUsings(EntityModel entity, ServiceModel serviceModel, ModuleModel module)
		{
			...
			_usings.AddLine(0, $"{_modelRoot.CommonNamespace}.DTOs");
			...
			_usings.AddLine(0, $"{module.DtoNamespace}.{entity.Name}");
			...
			if (serviceModel.InclCreate || serviceModel.UpdateMethods.Any() || serviceModel.ReadMethods.Any(m => m.UseRequest))
				_usings.AddIfNotExists($"{module.RequestNamespace}.{serviceModel.Version}.{entity.Name}");
		}
		...
		internal List<string> GenerateDeleteMapMethod(EntityModel entity, ServiceModel serviceModel)
		{
			...
			lines.AddLine(0, $"group.MapDelete(\"Delete{entity.Name}/{{id}}\", Delete{entity.Name})");
		...
		internal void GenerateDeleteMethod(EntityModel entity, ServiceModel serviceModel, string svcVarName, List<string> output)
		{
			...
			output.AddLine();
			output.AddLine(0, $"public static async Task<Result> Delete{entity.Name}(Guid id, I{entity.Name}Service {svcVarName})");
			output.AddLine(0, "{");
			output.AddLine(0 + 1, $"await {svcVarName}.Delete{entity.Name}(id);");
