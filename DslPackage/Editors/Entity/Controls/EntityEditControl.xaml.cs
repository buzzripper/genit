    ...
    public partial class EntityEditControl : UserControlBase
    {
        ...
        private void LoadFromModel()
        {
            ...
            try
            {
                ...
                chkSoftDelete.IsChecked = _entityModel.SoftDelete;
        ...
        private void chkSoftDelete_Changed(object sender, RoutedEventArgs e)
        {
            if (_isUpdating || _entityModel == null)
                return;
        ...
        private void dgProperties_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            if (checkbox != null && checkbox.IsEnabled)
            {
                ...
                {
                    // Block all checkbox toggles for RowVersion properties
        ...
        private void dgProperties_SelectedCellsChanged(object sender, SelectedCellsChangedEventArgs e)
        {
            ...
            if (sender is DataGrid grid && grid.CurrentCell != null && grid.CurrentCell.Column != null)
            {
                grid.Dispatcher.BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        var property = grid.CurrentItem as PropertyModel;
                        if (property != null)
                        {
                            if (property.IsRowVersion)
                                return;
                            if (property.IsAuditable)
                                return;
                            if (property.Name == "Id" && grid.CurrentCell.Column?.Header?.ToString() == "Name")
                                return;
                        }
