...
namespace Dyvenix.GenIt
{
	...
	public class AssociationAddRule : AddRule
	{
		...
		private void CreateSourceNavigationProperty(Association association, EntityModel source, EntityModel target)
		{
			...
			string navPropName = !string.IsNullOrEmpty(association.SourceRoleName)
				...
				: GetUniqueNavPropertyName(source, target.Name, association.TargetMultiplicity);
		...
		private void CreateTargetNavigationProperty(Association association, EntityModel target, EntityModel source)
		{
			...
			string navPropName = !string.IsNullOrEmpty(association.TargetRoleName)
				...
				: GetUniqueNavPropertyName(target, source.Name, association.SourceMultiplicity);
		...
		private string GetUniqueNavPropertyName(EntityModel entity, string baseName, Multiplicity multiplicity)
		{
			var name = baseName;
			...
			if (!entity.NavigationProperties.Any(np => np.Name == name))
				return name;

			int suffix = 2;
			while (entity.NavigationProperties.Any(np => np.Name == name + suffix))
			{
				suffix++;
			}
			return name + suffix;
	...
	/// <summary>
	/// Rule that fires when EntityModel.SoftDelete property changes.
	/// Automatically creates or deletes soft delete properties.
	/// </summary>
	[RuleOn(typeof(EntityModel), FireTime = TimeToFire.TopLevelCommit)]
	public class EntityModelSoftDeleteChangeRule : ChangeRule
	{
		internal static readonly string[] SoftDeletePropertyNames = new[]
		{
			"DeletedUtc",
			"DeletedByUserId"
		};
		...
		public override void ElementPropertyChanged(ElementPropertyChangedEventArgs e)
		{
			if (e.DomainProperty.Id != EntityModel.SoftDeleteDomainPropertyId)
				return;
			...
			if (newValue)
				CreateSoftDeleteProperties(entity);
			else
				DeleteSoftDeleteProperties(entity);
		}

		private void CreateSoftDeleteProperties(EntityModel entity)
		{
			int maxOrder = entity.Properties.Count > 0 ? entity.Properties.Max(p => p.DisplayOrder) : 0;

			CreateIfMissing(entity, "DeletedUtc", "DateTime", true, ref maxOrder);
			CreateIfMissing(entity, "DeletedByUserId", "Guid", true, ref maxOrder);
		}
		...
		private void DeleteSoftDeleteProperties(EntityModel entity)
		{
			var softDeleteProps = entity.Properties.Where(p => p.IsSoftDelete).ToList();
			foreach (var prop in softDeleteProps)
			{
				if (!prop.IsDeleting && !prop.IsDeleted)
					prop.Delete();
			}
	...
	/// <summary>
	/// Rule that fires when a PropertyModel is being deleted.
	/// If it's a soft delete property, sync back to EntityModel.SoftDelete.
	/// </summary>
	[RuleOn(typeof(PropertyModel), FireTime = TimeToFire.TopLevelCommit)]
	public class SoftDeletePropertyDeleteRule : DeletingRule
	{
		public override void ElementDeleting(ElementDeletingEventArgs e)
		{
			var property = e.ModelElement as PropertyModel;
