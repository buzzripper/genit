...
namespace Dyvenix.GenIt.SchemaImporter
{
	class Program
	{
		static int Main(string[] args)
		{
			...
			string modelName = args.Length > 2 ? args[2] : Path.GetFileNameWithoutExtension(outputPath);
			string schemaFilter = args.Length > 3 ? args[3] : null;

			// Ensure output has .gmdl extension
			if (!outputPath.EndsWith(".gmdl", StringComparison.OrdinalIgnoreCase))
			{
				outputPath += ".gmdl";
			}
