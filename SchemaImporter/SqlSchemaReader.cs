                    ...
                    using (var reader = colCmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            try
                            {
                                ...
                                columnInfo.DataType = reader.GetString(1);
                                columnInfo.MaxLength = reader.IsDBNull(2) ? (int?)null : (int)reader.GetInt32(2);
                                columnInfo.NumericPrecision = reader.IsDBNull(3) ? (int?)null : reader.GetByte(3);
                                columnInfo.NumericScale = reader.IsDBNull(4) ? (int?)null : (int)reader.GetInt32(4);
                                columnInfo.IsNullable = string.Equals(reader.GetString(5), "YES", StringComparison.OrdinalIgnoreCase);
                                columnInfo.IsPrimaryKey = primaryKeys.Contains(pkKey);
                                columnInfo.IsForeignKey = foreignKeys.Contains(fkKey);
                                columnInfo.IsIdentity = identityColumns.Contains(identityKey);
                                columnInfo.OrdinalPosition = reader.GetInt32(6);

                                table.Columns.Add(columnInfo);
                            }
                            catch (Exception ex)
                            {
                                Console.Error.WriteLine($"Table: {table.TableName} Column: {columnName}: {ex.Message}");
                                throw;
                            }
                        }
                    }
